function [geom, func, l1, l2] = geometry_NL(phi_o, la1_o, la2_o)
    syms phi la1 la2
    
    % Input geometric values.
    geom.r_1 = 113e-9;                  % Radius of tallest stereocilia (row 1). 
    geom.r_2 = 118e-9;                  % Radius of middle stereocilia (row 2).
    geom.r_3 = 82.45e-9;                % Radius of shortest stereocilia (row 3).
    
    geom.l1_gs = 2.3e-6;                % Height of the first tip link insertion point in row 1 stereocilia from its pivot point.
    geom.l2_gs = 1e-6;                  % Height of the second tip link insertion point in row 2 stereocilia from its pivot point.
    
    geom.r_CA = 4.2e-6;                 % Length of the tallest stereocilia.
    geom.r_BD = 2.2e-6 - geom.r_2;      % Length of the middle stereocilia minus its radius.
    geom.r_EF = 1.1e-6 - geom.r_3;      % Length of the shortest stereocilia minus its radius.
    ...
    l1 = sqrt((geom.l1_gs-(la1-la1_o))^2+(geom.r_BD+geom.r_2)^2+geom.r_1^2+geom.b_12^2+2*(geom.r_BD+geom.r_2) ...
        *(geom.l1_gs-(la1-la1_o))*cos(a1+a2)+2*geom.r_1*(geom.r_BD+geom.r_2)*sin(a1+a2)-2*geom.b_12 ...
        *((geom.r_BD+geom.r_2)*cos(a2)+geom.r_1*sin(a1)+(geom.l1_gs-(la1-la1_o))*cos(a1)));    
    
    e4 = diff(l1, phi);
    e5 = diff(l1, la1);
    
    a3 = acos(((geom.r_BD+geom.r_2)*cos(a2)+geom.r_1*sin(a1)-geom.b_12+(geom.l1_gs-(la1-la1_o))*cos(a1))/l1)-a1;
    ...
    l2 = sqrt((geom.l2_gs-(la2-la2_o))^2+(geom.r_EF+geom.r_3)^2+geom.r_2^2+geom.b_23^2+2*(geom.r_EF+geom.r_3) ...
        *(geom.l2_gs-(la2-la2_o))*cos(a4+a5)+2*geom.r_2*(geom.r_EF+geom.r_3)*sin(a4+a5)-2*geom.b_23 ...
        *((geom.r_EF+geom.r_3)*cos(a5)+geom.r_2*sin(a4)+(geom.l2_gs-(la2-la2_o))*cos(a4)));    
    
    e10 = diff(l2, phi);
    e11 = diff(l2, la2);
    
    a6 = acos(((geom.r_EF+geom.r_3)*cos(a5)+geom.r_2*sin(a4)-geom.b_23+(geom.l2_gs-(la2-la2_o))*cos(a4))/l2)-a4;
